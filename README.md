...
As you can see the original name was saved with a hash, so if you add a same file multi-times only the first will be save. The **originalNameStorage** will not clear automatically, use command `Clear unused original name storage`. This command will remove an entry if no linked attach's hash match the entry.
