	...
	async onload() {
			...
			checkCallback: (checking: boolean) => {
				...
				if (file) {
					...
					if (!checking) {
						...
						const { setting } = getOverrideSetting(this.settings, file, undefined);
		...
		this.registerEvent(
			...
			this.app.vault.on("rename", async (file: TAbstractFile, oldPath: string) => {
				...
				const { setting } = getRenameOverrideSetting(this.settings, file, oldPath, undefined);

				debugLog("rename - using settings:", setting);
				if (setting.type === SETTINGS_TYPES.FOLDER || setting.type === SETTINGS_TYPES.FILE) {
					updateOverrideSetting(this.settings, file, oldPath, undefined);
